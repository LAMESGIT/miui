...
public abstract class Flowable<T> implements Publisher<T> {
    ...
    public static Flowable<Long> intervalRange(long j, long j2, long j3, long j4, TimeUnit timeUnit, Scheduler scheduler) {
        ...
        if (i < 0) {
            ...
            if (j <= 0 || j5 >= 0) {
                ...
                return RxJavaPlugins.onAssembly(new FlowableIntervalRange(j, j5, Math.max(0, j6), Math.max(0, j4), timeUnit2, scheduler2));
            }
    ...
    public final Flowable<T> takeLast(long j, long j2, TimeUnit timeUnit, Scheduler scheduler, boolean z, int i) {
        ...
        ObjectHelper.verifyPositive(i2, "bufferSize");
        if (j3 >= 0) {
            return RxJavaPlugins.onAssembly(new FlowableTakeLastTimed(this, j3, j2, timeUnit2, scheduler2, i2, z));
        }
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("count >= 0 required but it was ");
    ...
    public final Flowable<Flowable<T>> window(long j, long j2, TimeUnit timeUnit, Scheduler scheduler, int i) {
        ...
        TimeUnit timeUnit2 = timeUnit;
        ObjectHelper.requireNonNull(timeUnit2, "unit is null");
        return RxJavaPlugins.onAssembly(new FlowableWindowTimed(this, j3, j4, timeUnit2, scheduler2, Long.MAX_VALUE, i2, false));
    }
