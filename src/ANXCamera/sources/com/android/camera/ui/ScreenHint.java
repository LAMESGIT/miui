...
public class ScreenHint {
    ...
    public void updateHint() {
        ...
        if (this.mStorageSpace == -1) {
            string = this.mActivity.getString(2131296267);
        } else if (this.mStorageSpace == -2) {
            string = this.mActivity.getString(2131296269);
        } else if (this.mStorageSpace == -3) {
            string = this.mActivity.getString(2131296270);
        } else if (this.mStorageSpace >= Storage.LOW_STORAGE_THRESHOLD) {
            string = null;
        } else if (Storage.isPhoneStoragePriority()) {
            string = this.mActivity.getString(2131296454);
        } else {
            string = this.mActivity.getString(2131296455);
        }
        if (string != null) {
    ...
    public void showObjectTrackHint() {
        ...
        editor.apply();
        RotateTextToast.getInstance(this.mActivity).show(2131296610, 0);
    }
    ...
    public void showFirstUseHint() {
        if (this.mSystemChoiceDialog == null || !this.mSystemChoiceDialog.isShowing()) {
            ...
            if (z) {
                ...
                if (b.go() && !contains && z) {
                    this.mSystemChoiceDialog = RotateDialogController.showSystemChoiceDialog(this.mActivity, this.mActivity.getString(2131296273), this.mActivity.getString(2131296274), this.mActivity.getString(2131296275), this.mActivity.getString(2131296276), new Runnable() {
                        public void run() {
