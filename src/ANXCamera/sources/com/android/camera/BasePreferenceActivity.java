...
public abstract class BasePreferenceActivity extends PreferenceActivity implements OnPreferenceChangeListener, OnPreferenceClickListener {
    ...
    public void onResume() {
        ...
        if (Util.isLabOptionsVisible()) {
            Toast.makeText(this, 2131296530, 1).show();
        }
    }
    ...
    private void filterByConfig() {
        ...
        boolean ft = DataRepository.dataItemFeature().ft();
        String str = TAG;
    ...
    private void filterByPreference() {
        ...
        if ((b.qj && !"normal".equals(videoSpeed)) || b.ie()) {
    ...
    private void filterByDeviceCapability() {
        if (this.mFromWhere == 172) {
            ...
            if (CameraSettings.getSupportedHfrSettings(0).contains(CameraSettings.SIZE_FPS_1080_240) || isSlowMotionFps120) {
                filterUnsupportedOptions(this.mPreferenceGroup, previewListPreference, Arrays.asList(getResources().getStringArray(2131623945)));
    ...
    public boolean onPreferenceClick(Preference preference) {
        if (preference.getKey().equals(PREF_KEY_RESTORE)) {
            RotateDialogController.showSystemAlertDialog(this, getString(2131296271), getString(2131296272), getString(17039370), new Runnable() {
                public void run() {
            ...
            if (CameraSettings.KEY_PRIORITY_STORAGE.equals(preference.getKey())) {
                ...
                RotateDialogController.showSystemAlertDialog(this, getString(2131296748), getString(2131296749), getString(2131296747), new Runnable() {
                    public void run() {
    ...
    private void restorePreferences() {
        ...
        resetSnapSetting();
        initializeActivity();
        PriorityStorageBroadcastReceiver.setPriorityStorage(getResources().getBoolean(2131689472));
        onSettingChanged(3);
    }
    ...
    private void updateEntries() {
        ...
        if (previewListPreference2 != null && Util.isAntibanding60()) {
            string = getString(2131296446);
            previewListPreference2.setValue(string);
            previewListPreference2.setDefaultValue(string);
        }
        if (checkBoxPreference != null) {
            checkBoxPreference.setChecked(getResources().getBoolean(CameraSettings.getDefaultPreferenceId(2131689475)));
        }
        if (previewListPreference3 != null) {
            Object string2;
            if (DataRepository.dataItemRunning().isSwitchOn("pref_camera_auto_zoom")) {
                string2 = getString(2131296288);
                previewListPreference3.setShowArrow(false);
                previewListPreference3.setEnabled(false);
            } else {
                string2 = getString(CameraSettings.getDefaultPreferenceId(2131296288));
                previewListPreference3.setShowArrow(true);
                previewListPreference3.setEnabled(true);
            }
            previewListPreference3.setDefaultValue(string2);
            previewListPreference3.setValue(string2);
        }
        if (previewListPreference4 != null && b.hm()) {
            String string3 = getString(2131296644);
            previewListPreference4.setDefaultValue(string3);
            previewListPreference4.setValue(string3);
            string3 = Secure.getString(getContentResolver(), "key_long_press_volume_down");
            if (System.getInt(getContentResolver(), "volumekey_wake_screen", 0) == 1 || "public_transportation_shortcuts".equals(string3) || "none".equals(string3)) {
                previewListPreference4.setValue(getString(2131296652));
                return;
            }
            string = DataRepository.dataItemGlobal().getString(CameraSettings.KEY_CAMERA_SNAP, null);
            if (string != null) {
                Secure.putString(getContentResolver(), "key_long_press_volume_down", CameraSettings.getMiuiSettingsKeyForStreetSnap(string));
                ...
                previewListPreference4.setValue(getString(2131296650));
            } else if ("Street-snap-movie".equals(string3)) {
                previewListPreference4.setValue(getString(2131296651));
            }
        }
    }
            ...
            for (int i = 0; i < preferenceCount; i++) {
                ...
                if (preference instanceof PreviewListPreference) {
                    ...
                    if ("pref_camera_picturesize_key".equals(previewListPreference.getKey())) {
                        previewListPreference.setValue(PictureSizeManager.getUserPictureSizeRatioString());
                    } else {
    ...
    private void updateConflictPreference(Preference preference) {
        if (b.qj && CameraSettings.isFrontCamera() && CameraSettings.isMovieSolidOn() && 6 <= CameraSettings.getPreferVideoQuality(this.mFromWhere)) {
            ...
            String string = getString(CameraSettings.getDefaultPreferenceId(2131296288));
