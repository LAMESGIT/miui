...
public class PostProcessor {
    ...
    private DataListener mDataListener = new DataListener() {
        public void onParallelDataAvailable(@NonNull CaptureData captureData) {
            String access$000 = PostProcessor.TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("[z] onParallelDataAvailable: ");
            ...
            if (Build.IS_DEBUGGABLE) {
                for (CaptureDataBean captureDataBean : captureData.getCaptureDataBeanList()) {
                    ...
                    String access$0002 = PostProcessor.TAG;
                    StringBuilder stringBuilder2 = new StringBuilder();
                    stringBuilder2.append("[z] Result timestamp: ");
            ...
            int algoType = captureData.getAlgoType();
            if (algoType != 4) {
                ImageFormat toImageQueueKey = ImagePool.getInstance().toImageQueueKey(((CaptureDataBean) captureData.getCaptureDataBeanList().get(0)).getMainImage());
                if (ImagePool.getInstance().isImageQueueFull(toImageQueueKey, 4)) {
                    Log.w(PostProcessor.TAG, "[z] wait image pool>>");
                    ImagePool.getInstance().waitIfImageQueueFull(toImageQueueKey, 4, 0);
                    Log.w(PostProcessor.TAG, "[z] wait image pool<<");
                }
    ...
    private ImageProcessorStatusCallback mImageProcessorStatusCb = new ImageProcessorStatusCallback() {
        public void onImageProcessed(Image image, int i) {
            ...
            String access$000 = PostProcessor.TAG;
            StringBuilder stringBuilder3 = new StringBuilder();
            stringBuilder3.append("[2] onImageProcessed: ");
            ...
            if (parallelTaskData != null) {
                if (PostProcessor.this.mCurrentAlgoType != 4) {
                    ...
                    Log.d(access$0002, String.format(Locale.ENGLISH, "[2] onImageProcessed: pictureSize: %s, imageSize: %s", objArr));
                ...
                stringBuilder.append("[2] onImageProcessed: captureResult = ");
    ...
    public class CaptureStatusListener {
        public void onCaptureStarted(@NonNull ParallelTaskData parallelTaskData) {
            ...
            String access$000 = PostProcessor.TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("[0] onCaptureStarted: timestamp = ");
            stringBuilder.append(timestamp);
            Log.d(access$000, stringBuilder.toString());
            PostProcessor.this.mParallelTaskHashMap.put(Long.valueOf(timestamp), parallelTaskData);
        ...
        public void onCaptureCompleted(ICustomCaptureResult iCustomCaptureResult, boolean z) {
            String access$000 = PostProcessor.TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("[0] onCaptureCompleted: timestamp = ");
    ...
    private class ImageAvailableListener implements OnImageAvailableListener {
        ...
        public void onImageAvailable(ImageReader imageReader) {
            ...
            String access$000 = PostProcessor.TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("[0] onImageAvailable: timestamp = ");
